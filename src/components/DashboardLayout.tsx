  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="lg:hidden">
        <div className="flex items-center justify-between p-4 bg-gray-800 shadow">
          <button
          ...
          <span className="text-lg font-semibold text-white">AI MEV Bot</span>
        </div>
        {isMobileMenuOpen && (
              ...
              <nav className="px-2 py-4">
                {menuItems.map((item) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    className="flex items-center px-4 py-2 mt-2 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                ...
                <button
                  ...
                  className="flex items-center w-full px-4 py-2 mt-2 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white"
                >
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
        <div className="flex flex-col flex-1 min-h-0 bg-gray-800 border-r border-gray-700">
          <div className="flex items-center flex-shrink-0 h-16 px-4">
            <span className="text-xl font-semibold text-white">AI MEV Bot</span>
          </div>
          <nav className="flex-1 px-3 py-4 space-y-1">
            {menuItems.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className="flex items-center px-4 py-2 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white"
              >
                <item.icon className="w-6 h-6 mr-3" />
                {item.name}
              </Link>
